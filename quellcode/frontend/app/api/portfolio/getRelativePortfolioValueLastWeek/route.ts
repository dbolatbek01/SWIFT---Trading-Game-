/**
 * API Route: Retrieve Last Week's Relative Portfolio Performance
 *
 ...
 *
 * @module api/portfolio/getRelativePortfolioValueLastWeek
 */
...
export const GET = async (request: NextRequest) => {
  try {
    ...
    const backendResponse = await fetch(
      `${BACKEND_BASE_URL}/getRelativePortfolioValueLastWeek/${authToken.accessToken}`,
      {
        method: 'GET',
    ...
    console.error('Failed to fetch last week\'s relative portfolio performance:', error);
    return NextResponse.json(
      {
        error: 'Failed to retrieve last week\'s portfolio performance',
