/**
 * API Route: Retrieve Last Month's Relative Portfolio Performance
 *
 ...
 *
 * @module api/portfolio/getRelativePortfolioValueLastMonth
 */
...
export const GET = async (request: NextRequest) => {
  try {
    ...
    const backendResponse = await fetch(
      `${BACKEND_BASE_URL}/getRelativePortfolioValueLastMonth/${authToken.accessToken}`,
      {
        method: 'GET',
    ...
    console.error('Failed to fetch last month\'s relative portfolio performance:', error);
    return NextResponse.json(
      {
        error: 'Failed to retrieve last month\'s portfolio performance',
